...
import random
import json
import backend as bk # Imports all the heavy lifting from your new file!
    ...
    with st.expander("✏️ Rename", expanded=False):
        ...
        if st.button("Save Rename", type="primary", use_container_width=True):
            ...
            if file_path != new_path:
                ...
                st.cache_data.clear() # Clear cache so home screen updates instantly
            ...
            if new_pdf and new_topic_name and selected_folder:
                with st.spinner("Generating database..."):
                    ...
                    if raw_data:
                        ...
                        st.cache_data.clear() # Fixes lag by refreshing home
                        bk.play_feedback('success'); st.success(f"✅ Success!"); time.sleep(1); st.rerun()
        ...
        for idx, folder_name in enumerate(library.keys()):
            with tabs[idx]:
                ...
                for c_idx, (topic_name, file_path) in enumerate(library[folder_name].items()):
                    with cols[c_idx % 3]:
                        # LAG FIX: Using lightweight columns instead of slow Popovers
                        ...
                        with c_menu:
                            if st.button("⚙️", key=f"menu_{file_path}", help="Options"):
